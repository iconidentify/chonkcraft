...
public final class BattleNetPathFinder {
    ...
    /**
     * Deep route entry point with saturated two-face buffer carry-over.
     *
     * @param retainSaturatedWallFaceHeads when the two saturated wall probes
     *     choose different headings, preserve the first probe's opening byte
     *     before the winning opposite probe's opening byte
     */
    public static PathFinder.Path find(int fromX, int fromY, int toX, int toY,
            int stride, Passability passability,
            Passability optimizationPassability, GoalMarker goalMarker,
            ...
            boolean reverseWallFaceOrder,
            boolean retainFirstWallFace,
            boolean retainSaturatedWallFaceHeads) {
    ...
    private static PathFinder.Path escapeObstacle(List<Integer> prefix,
        ...
        Candidate best = null;
        Integer firstWallFaceHeading = null;
        ...
        for (int turn : turns) {
            ...
            if (join == null) {
                if (System.getenv("CHONKCRAFT_TRACE_BNE_WALL") != null
                    ...
                    System.err.printf("JBNEWALL from=%d,%d goal=%d,%d "
                                    + "turn=%d failed route=%s shared=0%n",
                            startX, startY, toX, toY, turn,
                            partialHeadings(route, startX, startY, stride));
                }
                if (shareWallBufferBetweenFaces) {
            ...
            List<Integer> rawSteps = System.getenv(
                    "CHONKCRAFT_TRACE_BNE_WALL") == null
                    ? null
                    : partialHeadings(route, startX, startY, stride);
            ...
            if (routeLength <= 0) {
                if (rawSteps != null) {
                    System.err.printf("JBNEWALL from=%d,%d goal=%d,%d "
                                    + "turn=%d join=%d,%d raw=%s "
                                    + "optimized=invalid shared=%d%n",
                            startX, startY, toX, toY, turn,
                            join[0], join[1], rawSteps,
            ...
            if (steps == null || steps.isEmpty()) {
                if (rawSteps != null) {
                    System.err.printf("JBNEWALL from=%d,%d goal=%d,%d "
                                    + "turn=%d join=%d,%d raw=%s len=%d "
                                    + "optimized=disconnected shared=%d%n",
                            startX, startY, toX, toY, turn,
                            join[0], join[1], rawSteps, routeLength,
                            shareWallBufferBetweenFaces ? 1 : 0);
                }
                continue;
            }
            ...
            if (firstWallFaceHeading == null) {
                firstWallFaceHeading = headings.get(0);
            }
            if (System.getenv("CHONKCRAFT_TRACE_BNE_WALL") != null) {
                System.err.printf("JBNEWALL from=%d,%d goal=%d,%d turn=%d "
                                + "join=%d,%d raw=%s route=%s len=%d remaining=%d "
                                + "score=%d shared=%d%n",
                        startX, startY, toX, toY, turn,
                        join[0], join[1], rawSteps, headings, routeLength, remaining,
                        candidate.distance(),
                        shareWallBufferBetweenFaces ? 1 : 0);
            }
            if (best == null) {
        ...
        if (retainSaturatedWallFaceHeads
                && firstWallFaceHeading != null && best != null
                && best.headings().get(0) != firstWallFaceHeading) {
            // Both saturated probes write into the same native route buffer.
            // The first face's opening byte survives at index zero while the
            // shorter opposite face begins at index one. The rest of that
            // opposite trace is not part of the bounded recovery prefix.
            return found(List.of(
                    firstWallFaceHeading, best.headings().get(0)));
        }
