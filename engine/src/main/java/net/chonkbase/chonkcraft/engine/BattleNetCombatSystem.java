...
final class BattleNetCombatSystem {
     ...
     */
    private boolean retainedBuildingRetargetTail(Unit unit) {
    ...
    private boolean retainedBuildingRetargetReplay(Unit unit) {
        return retainedBuildingRetargetTail(unit)
                && unit.battleNetBuildingRetargetReplay();
    }
    ...
    private boolean openBattleNetSaturatedCardinalRouteTerminator(
        ...
        boolean wrappedCardinalColdRetry = attackStart >= 0
                && !settlingNow
        ...
        boolean saturatedCardinalRouteTerminator =
                ...
                && !Direction.isDiagonal(unit.lastStepHeading())
                && (wrappedCardinalColdRetry || recurringBlockedStillRetry
                        || (unit.battleNetAnimationTimer() == 1
                                && onBattleNetChaseMoveBody(unit)))
                && unit.target() != null
                && unit.target().isAlive()
        ...
        boolean ownsIdleDraw = wrappedCardinalColdRetry
                || ((settlingNow || recurringBlockedStillRetry)
                        && unit.routeSpent()
                        && unit.battleNetPathStepsTaken() > 0);
        if (!ownsIdleDraw && !wrappedCardinalColdRetry) {
            return false;
        }
        if (ownsIdleDraw) {
            world.idle.advanceBattleNetActiveOrderIdleRandom(unit);
        }
        if (wrappedCardinalColdRetry) {
            // A saturated wrapped route has already paid both complete wall
            // probes before its compact cardinal pair is written. When the
            // second byte parks, the following active-order visit clears the
            // packed collision generation and repeatedly constructs Attack
            // 3,2,1 while the direct cardinal face remains occupied. Each
            // timer-one wake still crosses FUN_0040ad30 and therefore owns an
            // asynchronous land-idle draw; it does not consume the separate
            // synchronized decision stream. It never accepts a free diagonal
            // side-step. XHuman 12 slot 1504 parks S,S on fixture 279, clears
            // 0x60 -> 0x00 on 280, and remains in this constructor loop
            // through at least fixture 300.
            if (unit.battleNetCollisionCounter() >= 6) {
        ...
        world.causalTrace.event(world.cycle,
                ...
                "wrapped_cold_retry", wrappedCardinalColdRetry);
        return true;
    }
    ...
    void stepAttack(Unit unit) {
        ...
        if (unit.battleNetBlockedChaseAttackConstruction()) {
            boolean retainedBuildingTail =
                    retainedBuildingRetargetTail(unit);
            boolean retainedBuildingReplay = retainedBuildingTail
                    && unit.battleNetBuildingRetargetReplay();
            ...
            boolean unofferedLongRetainedPark =
                    longRetainedPaidConstruction
            ...
            if (retainedBuildingTail && !retainedBuildingReplay) {
                // A long building retarget whose old route cursor was below
                // the replay boundary keeps its cached tail through Attack
                // construction. Timer one refreshes the nearest point inside
                // the target footprint and consumes the approved next byte in
                // this visit. XHuman 12 slot 1503 changes from a fourteen-byte
                // mobile tail to the guard tower, then commits cached SE on
                // fixture 271. Slot 1510's fifteen-byte counter-witness owns
                // the distinct RI20/redraw replay below.
                ...
                unit.setBattleNetBuildingRetargetReplay(false);
                int targetWidth = Math.max(1,
                        completedQuarryLeg.type().tileWidth());
                int targetHeight = Math.max(1,
                        completedQuarryLeg.type().tileHeight());
                unit.setPathGoal(
                        World.battleNetNearFootprintCoordinate(
                                unit.tileX(), completedQuarryLeg.tileX(),
                                targetWidth),
                        World.battleNetNearFootprintCoordinate(
                                unit.tileY(), completedQuarryLeg.tileY(),
                                targetHeight));
            ...
            int moveStart = world.battleNetSequence == null ? -1
                    : world.idle.battleNetSequenceStart(unit,
                            BattleNetSequence.MOVE_ANIMATION);
            if (unofferedLongRetainedPark) {
                // A four-byte mobile tail is retained through the complete
                // Move constructor only while a live hit offer owns the
                // cached route (Human 13 grunt 1507). With no offer, the
                // completed Attack constructor first parks that old buffer at
                // RI 20 and advances the formation-pressure generation. The
                // following Move visit draws and probes the replacement ray.
                // XHuman 12 slot 1517 is the sealed contrast: fixture 246 is
                // Move-start/1 with collision one and no route; fixture 247
                // writes SE,SE,SW,W and begins its fifteen-count refusal band.
                ...
                unit.setWaitCycles(0);
                unit.setBattleNetOrderDelay(0);
                // Stage three transfers the paid Attack constructor to the
                // replacement Move probe. It prevents that probe from
                // parking the freshly drawn cooperative route a second time;
                // the blocked head instead retains the ray and starts its
                // native fifteen-count band on the next visit.
                ...
                AnimationSet parkedSet = unit.type() == null
                        ? null : unit.type().animationSet();
                Animation parkedMove = parkedSet == null ? null
                        : parkedSet.get(AnimationSet.State.MOVE);
                if (parkedMove != null
                        && unit.animation().current() != parkedMove) {
                    unit.animation().switchTo(parkedMove);
                }
                return;
            }
            if (moveStart >= 0) {
        ...
        if (unit.chasing() && unit.canMove()) {
            ...
            if (chased != null && atChaseBoundary) {
                    ...
                    if (!world.canEnter(unit, pathn1X, pathn1Y)) {
                        ...
                        if (!pathn1FreeProgress && !pathn1ChangedTarget) {
                            ...
                            if (footprintNearBuilding) {
                                ...
                                unit.setWaitCycles(0);
                                unit.setBattleNetOrderDelay(0);
                                if (unit.battleNetPathInitialLength() > 2) {
                                    // A long building approach which exhausts
                                    // on the footprint skirt starts native's
                                    // next collision generation before the
                                    // replacement buffer is drawn. The nibble
                                    // survives the accepted first heading and
                                    // keeps that mover solid to later formation
                                    // writers (XHuman 12 slot 1508, fixtures
                                    // 270..283). A compact two-byte skirt has
                                    // no such carry (slot 1358 at fixture 57).
                                    unit.setBattleNetCollisionCounter(Math.min(14,
                                            unit.battleNetCollisionCounter() + 1));
                                }
                ...
                if (candidate != null && candidate != previous) {
                    ...
                    boolean completedSettledRefusalBand = keepPathn > 1
                            ...
                            && unit.battleNetAnimationTimer() == 1
                            && onBattleNetChaseMoveBody(unit);
                    boolean capacityPressureMobileRetarget =
                    ...
                    if (capacityPressureMobileRetarget) {
                        // A full route buffer which has already advanced
                        // several bytes is not a newly completed refusal-band
                        // writer. Its lone pressure generation belongs to the
                        // old buffer while AutoSelectTarget replaces the
                        // mobile quarry. Native therefore redraws with the
                        // ordinary hard formation view and probes the new head
                        // immediately; it does not soft-clear the rank and
                        // manufacture another cooperative wait. XHuman 12
                        // slot 1501 consumes five bytes of a capacity route,
                        // changes knight 1475 to footman 1477, writes
                        // N,NE,SE,NE,E,E,... and commits N on fixture 258.
                        completedSettledRefusalBand = false;
                    }
                    ...
                    boolean saturatedSettledResidualRetarget =
                            ...
                            && unit.battleNetCollisionCounter() >= 4
                            && unit.battleNetRefusals() == 0;
                    boolean saturatedSingleStepMobileRetarget =
                            settledMeleeResidualRetarget
                            && keepPathn
                                    == BattleNetPathFinder.MAX_PATH - 1
                            && unit.battleNetPathInitialLength()
                    ...
                    clearCollisionAfterSettledRetargetStep =
                            ...
                            || capacityPressureMobileRetarget
                            || (settledMeleeResidualRetarget
                                && unit.battleNetCollisionCounter() > 0
                                && unit.battleNetRefusals() == 0
                                && (unit.battleNetPathStepsTaken() == 1
                    ...
                    boolean longPaidBuildingRetarget =
                            settledMeleeResidualRetarget
                            && keepPathn >= 6
                            && unit.battleNetCollisionCounter() > 1
                            && unit.battleNetCollisionCounter() < 4
                    ...
                    boolean collisionOwnsRetargetConstruction =
                            ((((unit.battleNetRefusals() == 0
                                ...
                                // long retained tail switching to a building
                                // is redrawn immediately (XHuman 12 slots 1503
                                // and 1480 retain fourteen bytes when they
                                // switch to guard towers at fixtures 252 and
                                // 264). A hard-refused mobile route earns it
                                // in the compact buffer or paid collision band.
                                // Generations four and above have already paid
                                // and commit directly.
                                && unit.battleNetCollisionCounter() > 1
                                && unit.battleNetCollisionCounter() < 4
                                && !longPaidBuildingRetarget
                    ...
                    boolean pressuredResidualRetargetConstruction =
                            ...
                            && world.battleNetSequence != null;
                    boolean retainedBuildingReplayTransaction =
                            settledMeleeResidualRetarget
                            && keepPathn >= 15
                            && keepPathn
                                    < BattleNetPathFinder.MAX_PATH - 1
                    ...
                    if (keepPrefix) {
                        ...
                        // Target replacement erases the old logical route
                        // provenance. Preserve the one native cursor boundary
                        // which decides whether the later Attack-one wake
                        // parks at RI20 or spends the cached diagonal directly.
                        unit.setBattleNetBuildingRetargetReplay(
                                retainedBuildingReplayTransaction);
                        chased = candidate;
                        unit.setRouteSpent(false);
                        ...
                        if (completedSettledRefusalBand) {
                            ...
                            if (incumbentPlanBeforePaidRefillRetarget) {
                                ...
                                boolean saturatedParkedRetarget =
                                        ...
                                        && unit.battleNetCollisionCounter()
                                                >= 5
                                        && unit.battleNetRefusals() > 0;
                                boolean retainedPaidBuildingMobileFace =
                                        replacementAfterPaidBand
                                        && unit.battleNetCollisionCounter() == 0
                                        // Completed odd refusal generations
                                        // retain the collided building-assault
                                        // rank as a wall for the mobile
                                        // replacement. The intervening even
                                        // generation has crossed the replay
                                        // boundary and redraws with that rank
                                        // cooperative-soft. XHuman 12 slots
                                        // 1480/1506 carry generations one/three;
                                        // slot 1479 is the generation-two
                                        // counter-witness.
                                        && (unit.battleNetRefusals() & 1) == 1
                                        && previous.type() != null
                                        && previous.type().building()
                                        && chased.type() != null
                                        && !chased.type().building();
                                    ...
                                    world.planTowardsAfterRefusalBand(
                                            ...
                                            (Direction.isDiagonal(
                                                    ...
                                                    || (keepPathn >= 19
                                                            ...
                                                            && chased.type()
                                                                    .building())
                                                    || retainedPaidBuildingMobileFace);
                                }
                                world.actionSettledMeleeReplacementBroadRoute =
                                        replacementThroughCollidedMover;
                                // NewPath owns the whole replacement buffer,
                                // including its first byte. A matching old
                                // diagonal can make that look like a transfer:
                                // XHuman 12 slot 1508's fresh route itself
                                // begins NE at fixture 72. The counterexample
                                // is slot 1501 at fixture 277: its paid old NE
                                // tail survives Attack 3,2,1, but NewPath
                                // writes an SE-led route back to the knight.
                                // Overwriting that fresh head with the stale
                                // diagonal sends the grunt north of both the
                                // native route and its target.
                            ...
                            } else if (saturatedSingleStepMobileRetarget) {
                                // A saturated buffer may pay its complete Move
                                // band before consuming only one heading. Its
                                // collision generation remains live while the
                                // separate Java refusal proxies on departing
                                // formation mates no longer represent native's
                                // packed collision nibble. The replacement
                                // writer clears those zero-collision Move
                                // bodies for wall traversal, while the route
                                // optimizer still tests its ordinary hard
                                // occupancy view. XHuman 12 slot 1492 thereby
                                // writes the twenty-byte E-led outer wall and
                                // refuses that occupied E on fixture 271.
                                world.planTowardsAfterSaturatedSingleStepBand(
                                        unit, chased);
                                // The old saturated nibble paid for the writer
                                // above. The newly written E head starts its
                                // own generation when the physical occupancy
                                // probe refuses it later in this same visit
                                // (native raw 0x80 -> 0x10).
                                unit.setBattleNetCollisionCounter(0);
                                unit.setBattleNetRefusals(0);
                            } else {
                                world.planTowards(unit, chased, true);
                        ...
                        && !Direction.isDiagonal(
                                unit.lastStepHeading())
                        && !World.battleNetRangedChaseUnit(unit)) {
                    int first = unit.battleNetParkedRefusalHeading();
                    int direct = World.battleNetFirstBresenhamHeading(
                            unit.tileX(), unit.tileY(),
                            chased.tileX(), chased.tileY());
                    int turn = Math.floorMod(first - direct,
                            Direction.COUNT);
                    if (!Direction.isDiagonal(direct)
                            && (turn == 1
                                    || turn == Direction.COUNT - 1)) {
                        int opposite = Math.floorMod(
                                direct - (turn == 1 ? 1 : -1),
                                Direction.COUNT);
                        // The paid wake rejected this diagonal on the prior
                        // callback and advanced its native cursor to twenty.
                        // The next writer retries that proved face and keeps
                        // the opposite diagonal behind it as one compact wall
                        // pair. XHuman 12 slot 1517 writes SE,SW and commits
                        // SE on fixture 263; a fresh optimizer escapes west.
                        unit.setPath(new PathFinder.Path(
                                PathFinder.Result.FOUND,
                                new int[] {opposite, first}));
                    ...
                    int parkedRefusalHeading =
                            unit.battleNetParkedRefusalHeading();
                    boolean retainedCapacityWallContinuation =
                    ...
                    if (paidOneStepRefusalApproachSettled) {
                        ...
                        world.causalTrace.event(world.cycle,
                                ...
                                "rewrite_occupancy",
                                        unit.battleNetRetainedRewriteOccupancy(),
                                "last_heading", unit.lastStepHeading());
                        ...
                        boolean retainedAttackPromotion =
                                ...
                                && settledHeading >= 0
                                && (settledHeading & 1) != 0;
                        boolean coldDiagonalPromotion =
                                retainedAttackPromotion
                                && !unit.battleNetPaidRefusalRecoveryApproach();
                        unit.clearPath();
                        unit.setWaitCycles(0);
                        unit.setBattleNetOrderDelay(0);
                        unit.setBattleNetChaseEmptyRouteReplan(false);
                        if (coldDiagonalPromotion) {
                            // A diagonal admitted by the explicit recovery
                            // probe remains queued Move work. A diagonal
                            // reached without that paid-probe provenance has
                            // instead returned to active-order Still: retail
                            // pays 0040AD58, clears the packed refusal after
                            // the dispatch, and repeats Attack 3,2,1 while no
                            // compass square closes range. XHuman 12 slot
                            // 1481 is the sealed witness at fixtures 284,
                            // 287, and 290. Slot 1495's otherwise identical
                            // southwest settlement retains paid recovery and
                            // therefore stays on the draw-free Move arm.
                            unit.setRouteSpent(false);
                            unit.setBattleNetDirectRefusalRecoveryProbe(false);
                            unit.setBattleNetChaseEmptyRouteReplan(true);
                            unit.setBattleNetColdNoProgressRefusalLoop(true);
                    ...
                    boolean collidedResidualRefill =
                            (unit.battleNetChaseEmptyRouteReplan()
                                    || paidCollisionFourImmediateRefill)
                            && unit.battleNetCollisionCounter() > 0;
                    boolean saturatedWrappedCapacityRefill =
                            unit.battleNetAttackWrapDestArmPending()
                            && unit.battleNetChaseEmptyRouteReplan()
                            && unit.pathLength() == 0
                            && unit.battleNetCollisionCounter() == 5
                    ...
                    } else if (saturatedWrappedCapacityRefill) {
                        // A completely full wrapped route which parks its
                        // second cached face has already paid both the target
                        // decision and the formation boundary. Its next native
                        // writer is a compact two-byte direct ray; it does not
                        // reopen the ordinary twenty-byte wall optimizer.
                        // XHuman 12 slot 1504 writes S,S at fixture 263, then
                        // commits the first S while slot 1490 vacates the
                        // neighbouring diagonal. Replanning around live bodies
                        // instead writes a capacity NE ray and steals the cell
                        // needed by slot 1517's compact SE,SW pair.
                        int direct = World.battleNetFirstBresenhamHeading(
                                unit.tileX(), unit.tileY(),
                                chased.tileX(), chased.tileY());
                    ...
                    } else if (retainedCapacityWallContinuation) {
                        // The marker carried the parked face across callbacks;
                        // it must not also enable the older collision-four
                        // beginning-of-visit projection inside this writer.
                        unit.setBattleNetPaidLongResidualRefill(false);
                        world.planTowardsAfterRetainedCapacityWake(
                                unit, chased);
                    } else if (collidedResidualRefill) {
                        world.planTowardsAfterCollidedResidualRefill(
                                unit, chased);
                    } else {
                        world.planTowards(unit, chased, false);
                    }
                    if (retainedCapacityWallContinuation) {
                        // The retained face is an input to this one NewPath
                        // writer, not provenance of the replacement route.
                        // Leaving it armed lets a much later refill preserve
                        // an obsolete beginning-of-visit occupancy view.
                        unit.setBattleNetPaidLongResidualRefill(false);
                    }
                    int parkedRefusalX = parkedRefusalHeading >= 0
                            && parkedRefusalHeading < Direction.COUNT
                                    ? unit.tileX() + Direction.deltaX(
                                            parkedRefusalHeading)
                                            * world.battleNetMovementStride(unit)
                                    : Integer.MIN_VALUE;
                    int parkedRefusalY = parkedRefusalHeading >= 0
                            && parkedRefusalHeading < Direction.COUNT
                                    ? unit.tileY() + Direction.deltaY(
                                            parkedRefusalHeading)
                                            * world.battleNetMovementStride(unit)
                                    : Integer.MIN_VALUE;
                    Unit parkedRefusalBlocker = world.unitAt(
                            parkedRefusalX, parkedRefusalY);
                    boolean occupiedPaidRefusalFace =
                            paidCollisionFourImmediateRefill
                            && parkedRefusalBlocker != null
                            && parkedRefusalBlocker != unit
                            && parkedRefusalBlocker.isOnMap()
                            && !parkedRefusalBlocker.isDying()
                            && world.isAllied(unit.player(),
                                    parkedRefusalBlocker.player());
                    if (parkedRefusalHeading >= 0
                            && parkedRefusalHeading < Direction.COUNT
                            // When the paid face remains occupied, NewPath's
                            // hard-square wall route is already the native
                            // continuation. Rotating its head clockwise would
                            // reinsert the refused direction's blocked terrain
                            // component. XHuman 12 slot 1496 keeps N,NE,SE...
                            // at fixture 304; slot 1504's vacated-face refill
                            // retains the older clockwise promotion.
                            && !occupiedPaidRefusalFace
                            && !unit.battleNetAttackWrapDestArmPending()) {
                        ...
                        boolean freshBufferAlreadyCarriesParkedFace =
                                unit.pathLength() == 3
                                ...
                                && unit.peekHeadingAtDepth(1)
                                        == parkedRefusalHeading;
                        if (unit.pathLength() > 0
                                && !freshBufferAlreadyCarriesParkedFace) {
                            world.causalTrace.event(world.cycle,
                            ...
                            if (retainedCapacityWallContinuation
                                    ...
                                    && unit.peekHeadingAtDepth(1)
                                            == continuedHeading) {
                                // NewPath first writes the ordinary wall
                                // opener, then the paid cursor promotes its
                                // retained face ahead of that byte. This is a
                                // two-byte rotation, not a destructive head
                                // replacement: the original opener remains
                                // the second cached heading.
                                int ordinaryOpener = unit.peekHeading();
            ...
            if (paidConstructionMoveProbe) {
                if (!unit.isMoving()
                        ...
                        && unit.target().type() != null
                        && !unit.target().type().building()) {
                    // A blocked first probe still belongs to Move start. The
                    // movement animation briefly advances to its body while
                    // testing that heading; the refusal callback rewinds the
                    // visible cursor and exposes the complete 15..1 band.
                    // XHuman 12 slot 1517 is Move@2482/15 on fixture 247,
                    // never Move@2485/1 under a hidden Java-only delay.
                    int moveStart = world.idle.battleNetSequenceStart(unit,
                            BattleNetSequence.MOVE_ANIMATION);
                    if (moveStart >= 0) {
                        unit.setBattleNetSequenceOffset(moveStart);
            ...
            if (exhaustedMeleeApproach && world.actionMoveWalked) {
                if (pathLengthBeforeChaseWalk == 0
                    ...
                    unit.setWaitCycles(0);
                    unit.setBattleNetOrderDelay(0);
                    if (unit.battleNetAiBehavior() == 1
                            && unit.battleNetCollisionCounter() > 0
                            && unit.battleNetRefusals() == 0
                            && emptyApproachTarget.type() != null
                            && !emptyApproachTarget.type().building()) {
                        unit.setBattleNetColdNoProgressRefusalLoop(true);
                    }
                    if (rearmBattleNetHardRefusalAttack(unit)) {
                        return;
            ...
            if (exhaustedMeleeApproach) {
                if (unit.battleNetAiBehavior() == 1
                        ...
                        && emptyApproachTarget.type() != null
                        && !emptyApproachTarget.type().building()) {
                    // Only an actually empty replacement route enters the
                    // recurring direct-decision constructor. A residual park
                    // whose replacement planner succeeds remains outside;
                    // XHuman 12 slot 1496 therefore still redraws NE on
                    // fixture 107 despite identical local target geometry.
                    unit.setBattleNetColdNoProgressRefusalLoop(true);
                }
                if (rearmBattleNetHardRefusalAttack(unit)) {
                    return;
    ...
    private boolean armBattleNetBlockedChaseAttackConstruction(Unit unit) {
        ...
        boolean saturatedPaidEmptyResidualConstruction =
                ...
                && !unit.target().type().building()
                && !World.battleNetRangedChaseUnit(unit);
        boolean retainedCapacityPaidWake = unit != null
                && unit.type() != null
                && unit.type().maxAttackRange() <= 1
                && unit.chasing() && !unit.isMoving() && unit.stepDrained()
                ...
                && !unit.target().type().building()
                && !world.targets.inAttackRange(unit, unit.target());
        if (retainedCapacityPaidWake) {
            // A capacity route which changed mobile quarry on its opening leg
            // first retains eighteen bytes through a complete Move band. Its
            // timer-one wake is a route-index park, not another Attack-four
            // constructor: the next visit owns NewPath and may buy the next
            // cooperative band. XHuman 12 slot 1496 advances collision one to
            // two and parks E at fixture 271, then writes its nineteen-byte
            // replacement at fixture 272. Routing this state through the
            // generic blocked-chase constructor delayed NewPath three visits.
            int parkedHeading = unit.peekHeading();
    ...
    private boolean stepBattleNetAttackRefusalRecovery(Unit unit) {
        ...
        if (stage == 5) {
            ...
            boolean coldEmptyMobileDirectBand =
                    ...
                    && !World.battleNetRangedChaseUnit(unit);
            if (coldEmptyMobileDirectBand) {
                int direct = World.battleNetFirstBresenhamHeading(
                        unit.tileX(), unit.tileY(),
                        boxedTarget.tileX(), boxedTarget.tileY());
                int stride = world.battleNetMovementStride(unit);
                boolean directOpen = direct >= 0
                        && direct < Direction.COUNT
                        && world.canEnter(unit,
                                unit.tileX()
                                        + Direction.deltaX(direct) * stride,
                                unit.tileY()
                                        + Direction.deltaY(direct) * stride);
                if (!directOpen) {
                    // A cold empty chase constructor grants one fresh direct
                    // decision, not an arbitrary closer wall face. If that
                    // ray remains occupied, active-order Still owns its idle
                    // draw and another Attack 3,2,1 constructor. XHuman 12
                    // slot 1490 repeats this loop from fixtures 296 through
                    // 340 even though south is free and closer; its direct
                    // southwest square opens and commits only on 341.
                    return rearmBattleNetHardRefusalAttack(unit);
                }
                // The direct square has opened. Its constructor ownership is
                // consumed before stage six gives the ordinary route writer
                // the successful Move probe.
                unit.setBattleNetColdNoProgressRefusalLoop(false);
            }
    ...
    void finishBattleNetAttackSequenceMarker(Unit unit) {
        ...
        if (World.TRACE_MOVING != null
                && unit.id() == World.TRACE_MOVING_ID) {
            StringBuilder tracedRoute = new StringBuilder();
            for (int depth = 0; depth < unit.pathLength(); depth++) {
                tracedRoute.append(unit.peekHeadingAtDepth(depth));
            }
            System.err.printf("JATTACKMARKER cycle=%d unit=%d had=%d order=%s "
                            + "seq=%d/%d chase=%d path=%d moving=%d wait=%d "
                            + "delay=%d blocked=%d replan-hold=%d stage=%d "
                            + "target=%d offer=%d ai=%d initial=%d steps=%d "
                            + "last=%d collision=%d refusals=%d route-park=%d "
                            + "park-refill=%d park-steps=%d refusal-hold=%d "
                            + "wrap=%d path-bytes=%s%n",
                    ...
                    unit.battleNetPathInitialLength(),
                    unit.battleNetPathStepsTaken(), unit.lastStepHeading(),
