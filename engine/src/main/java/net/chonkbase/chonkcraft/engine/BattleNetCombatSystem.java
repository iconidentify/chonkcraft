...
final class BattleNetCombatSystem {
    ...
    void stepAttack(Unit unit) {
        ...
        if (unit.battleNetResidualEmptyApproachIdlePending()) {
            ...
            if (unit.battleNetSaturatedResidualFaceRetry()
                ...
                // ordinary active-order Still callback. That callback owns an
                // asynchronous idle draw before Attack construction. The
                // authenticated ledger places slot 1495's draw between slot
                // 1468's idle and the following projectile damage at fixture
                // 122; suppressing it reassigned every later missile and melee
                // roll. State remains 0x50 while NW drains, 0x60/Move-start-1
                // on fixture 121, and 0x00/Attack-start-3 on fixture 122. A
                // cardinal tail which was generated after refusal eight owns
                // the same completed-pressure handoff at a lower collision
                // generation: XHuman 12 slot 1501 parks nineteen north-led
                // bytes at collision three on fixture 157, then clears both
                // counters and enters Attack-start/3 on fixture 158.
                unit.setBattleNetSaturatedResidualFaceRetry(false);
                unit.setBattleNetSaturatedTerminatorRoutePending(true);
        ...
        if (unit.battleNetOrderDelay() > 0) {
            ...
            if (unit.chasing() && unit.pathLength() == 1
                ...
                if (unit.battleNetOrderDelay() == 1
                        && unit.battleNetRefusalHold()
                        && unit.battleNetRefusals() > 0
                        && unit.battleNetPathStepsTaken() == 0
                        && !unit.battleNetChaseEmptyRouteReplan()) {
                    // The complete hard-refusal band ends by parking its
                    // retained one-byte probe at timer one. The next Move
                    // callback draws and consumes a fresh route; it does not
                    // spend another refusal visit first. XHuman 12 slot 1506
                    // exposes RI20/Move-start-1 at fixture 167, then refills
                    // NW and commits it at fixture 168.
                    // An empty-route refill owns its fresh byte and consumes
                    // it instead (slot 1512's S at fixture 109).
                    unit.setBattleNetOrderDelay(0);
                    unit.clearPath();
                    unit.setRouteSpent(false);
                    // Preserve the saturated wall buffer which selected this
                    // paid route generation. Its shared face is what yields
                    // native's NW refill instead of the ordinary SE escape.
                ...
                boolean paidWakeHeadFree = false;
        ...
        boolean swung = false;
        // script.bin is the authoritative animation for a BNE attack.  When
        // its tail reaches OP0, CheckIfGoalValid runs on that visit even if
        // the parallel presentation animation still has Unbreakable set.
        // Letting the renderer-side swing veto this boundary leaves an
        // attacker pinned to a corpse until the visual script happens to
        // finish. XHuman 12 exposes the same error simultaneously on two
        // grunts and an axethrower at fixture 181: native changes all three
        // Attack orders to Still on the OP0 visit while Java kept swinging.
        ...
        if (unit.chasing() && unit.canMove()) {
            ...
            if (saturatedWallFacePairSettle) {
                // Attack owns the pixel drain before it calls the borrowed
                // Move executor, so this shared-buffer boundary is visible
                // here rather than in stepMove. Park route index twenty and
                // leave the retained opposite face for the next callback.
                int retainedHeading = unit.peekHeading();
            ...
            if (world.actionMoveWalked && !unit.isMoving()
                ...
                if (!inRange || movingQuarry) {
                    ...
                    if (spentSingleReplanConstruction) {
                        // A spent one-byte replacement has no live Move tail
                        // for the surrogate order delay to protect. Native
                        // exposes and drains Attack 3,2,1 directly, then lets
                        // timer one select the replacement target and step in
                        // that same callback. XHuman 12 slot 1476 / Java 124
                        // is 3,2,1 on fixtures 124..126 and moves north on
                        // 127. Freezing timer three behind order-delay two
                        // postponed the whole engagement handoff.
                        unit.setBattleNetOrderDelay(0);
            ...
            boolean shortCollidedAttackTail = unit.battleNetAiBehavior() == 1
                    ...
                    && unit.pathLength() == 3
                    && unit.battleNetCollisionCounter() == 1;
            boolean fullAttackRouteTail =
                    unit.pathLength() == BattleNetPathFinder.MAX_PATH - 1
                    && unit.battleNetCollisionCounter() == 0
                    // This is the resumed route after a settled residual
                    // exhausted into Attack construction. Fresh saturated
                    // routes have the same visible length and wrapper marker
                    // but continue through their second heading (XHuman 12
                    // slots 1495/1501 at fixture 141).
                    && unit.battleNetRetargetResidualParkRefill();
            if (world.actionMoveWalked && !unit.isMoving()
                    && unit.battleNetAttackWrapDestArmPending()
                    && unit.battleNetPathStepsTaken() == 1
                    && (shortCollidedAttackTail || fullAttackRouteTail)
                    && unit.target() != null && unit.target().isAlive()
                    && !world.targets.inAttackRange(unit, unit.target())) {
                // A saturated, but otherwise uncollided, replacement route
                // has the same paid-construction ownership boundary. Native
                // XHuman 12 slot 1476 lays twenty N-prefixed headings and
                // spends the first N on fixture 127, then parks the nineteen
                // retained bytes behind Attack 2539/3 on fixture 143. Letting
                // Move spend the second N here made formation units run
                // through their combat checkpoint and separate from the
                // engagement.
                world.armBattleNetAttackStart(unit);
                unit.setBattleNetAttackRefusalRecoveryStage(2);
                AnimationSet set = unit.type() == null
            ...
            int carryRefusalCollisionAfterSingleStep = 0;
            ...
            if (chased != null && atChaseBoundary) {
                ...
                if ((!unit.isMoving() || !attackOwnsResidual)
                    ...
                    if (deferBattleNetResidualRetargetSyncRand(
                ...
                if (candidate != null && candidate != previous) {
                    ...
                    boolean replacementOwnsFullRouteBuffer =
                            unit.battleNetPathInitialLength()
                                    == BattleNetPathFinder.MAX_PATH
                            && keepPathn
                                    + unit.battleNetPathStepsTaken()
                    ...
                    // A fully consumed route has no cached heading left, but
                    // its final pixels can still settle on the same visit
                    // that target scan installs a replacement quarry.  A
                    // single paid cooperative collision belongs to the old
                    // route in that case: NewPath uses it while drawing the
                    // replacement ray, then clears it when the first new
                    // heading commits. XHuman 12 native slot 1453 carries
                    // collision one through its c21..67 route, changes tower
                    // to knight at c68, steps SE, and writes 0x10 -> 0x00.
                    // Keeping the stale bit makes the later slot-1463 follower
                    // reject this visibly moving ally as non-cooperative and
                    // leave formation at c124.
                    ...
                    boolean settledResidualHeadFree = keepPathn > 0
                            && world.canEnter(unit,
                                    ...
                                    unit.tileY() + Direction.deltaY(
                                            unit.peekHeading())
                                            * world.battleNetMovementStride(unit));
                    // Consuming the only byte in a chase route closes that
                    // Move program completely. If target scan replaces the
                    // quarry on the residual-settle visit, retail enters a
                    // fresh Attack constructor before it is allowed to draw
                    // the replacement route. A longer exhausted route still
                    // returns through the live Move callback and may replace
                    // immediately (slot 1453 at fixture 68); the one-byte
                    // route is the cold boundary. XHuman 12 slot 1470 spends
                    // E, changes tower -> footman at fixture 124, exposes
                    // Attack-start 3,2,1, and first-steps N at fixture 127.
                    ...
                    // Behavior-one guards keep a long approved mobile route
                    // until a strictly better combatant appears. When that
                    // replacement is accepted on a residual boundary after
                    // at least two route bytes, native discards the incumbent
                    // tail through active-order Still and starts Attack 3,2,1.
                    // This is distinct from an equal-score spatial reorder,
                    // which may install its replacement ray immediately.
                    // XHuman 12 slot 1468 / Java 132 changes footman -> knight
                    // at fixture 119 and owns the async draw between the
                    // surrounding idle consumers.
                    boolean behaviorOneStrictMobileUpgradeConstruction =
                            settledMeleeResidualRetarget
                            && unit.battleNetAiBehavior() == 1
                            && unit.battleNetPathStepsTaken() >= 2
                            // A full twenty-byte route remains live state in
                            // BNE: replacing its quarry draws and commits the
                            // new route in this same callback. Only a compact
                            // incumbent tail returns through Attack 3,2,1.
                            && unit.battleNetPathInitialLength()
                                    < BattleNetPathFinder.MAX_PATH
                            // Generation four has already paid the formation
                            // pressure which owns this boundary. Native then
                            // accepts the better mobile quarry and lays its
                            // replacement route immediately; only generations
                            // two and three restart Attack construction.
                            && unit.battleNetCollisionCounter() < 4
                            // A stale attack-back offer is the ownership
                            // token for this callback. Ordinary periodic scans
                            // replace directly, and an in-range band change
                            // enters combat rather than reconstructing chase.
                            && unit.offeredTarget() != null
                    ...
                    // Pressure has to own the route, not merely be nearby. An
                    // unrefused route earns that ownership after two consumed
                    // headings; a hard-refused route earns it in the compact
                    // buffer or paid-band collision state. Generations four
                    // and above have already paid formation pressure, so a
                    // successful replacement ray clears and commits directly.
                    ...
                    int oldReplacementHeading = keepPathn > 0
                            ? unit.peekHeading() : -1;
                    int singlePaidLongRouteHeading = oldReplacementHeading;
                    if (previous != null && oldReplacementHeading >= 0
                            && !Direction.isDiagonal(
                                    oldReplacementHeading)) {
                        // The Java wall route can expose only the cardinal
                        // component of BNE's approved full-buffer face. Recover
                        // the missing secondary axis from the incumbent goal:
                        // E toward a quarry below is BNE's retained SE, for
                        // example. This is an ownership reconstruction, not a
                        // fresh path choice; require the diagonal to remain
                        // free and at least as progressive as the cardinal.
                        int mixedX = Direction.deltaX(
                                oldReplacementHeading);
                        int mixedY = Direction.deltaY(
                                oldReplacementHeading);
                        if (mixedX == 0) {
                            mixedX = Integer.signum(previous.tileX()
                                    - unit.tileX());
                        }
                        if (mixedY == 0) {
                            mixedY = Integer.signum(previous.tileY()
                                    - unit.tileY());
                        }
                        int mixed = Direction.fromDelta(mixedX, mixedY);
                        if (mixed >= 0 && Direction.isDiagonal(mixed)) {
                            int stride = world.battleNetMovementStride(unit);
                            int mixedTileX = unit.tileX()
                                    + Direction.deltaX(mixed) * stride;
                            int mixedTileY = unit.tileY()
                                    + Direction.deltaY(mixed) * stride;
                            int oldTileX = unit.tileX()
                                    + Direction.deltaX(oldReplacementHeading)
                                            * stride;
                            int oldTileY = unit.tileY()
                                    + Direction.deltaY(oldReplacementHeading)
                                            * stride;
                            int mixedDistance = Math.max(
                                    Math.abs(previous.tileX() - mixedTileX),
                                    Math.abs(previous.tileY() - mixedTileY));
                            int oldDistance = Math.max(
                                    Math.abs(previous.tileX() - oldTileX),
                                    Math.abs(previous.tileY() - oldTileY));
                            if (world.canEnter(unit, mixedTileX, mixedTileY)
                                    && mixedDistance <= oldDistance) {
                                singlePaidLongRouteHeading = mixed;
                            }
                        }
                    }
                    boolean singlePaidLongRouteHeadTransfer =
                            settledMeleeResidualRetarget
                            && unit.battleNetPathInitialLength()
                                    == BattleNetPathFinder.MAX_PATH
                            && unit.battleNetPathStepsTaken() >= 4
                            && keepPathn + unit.battleNetPathStepsTaken()
                                    == BattleNetPathFinder.MAX_PATH
                            && unit.battleNetCollisionCounter() == 1
                            && unit.battleNetRefusals() == 1
                            && singlePaidLongRouteHeading >= 0
                            && Direction.isDiagonal(
                                    singlePaidLongRouteHeading);
                    int approvedReplacementHeading =
                            singlePaidLongRouteHeadTransfer
                                    ? singlePaidLongRouteHeading
                                    : oldReplacementHeading;
                    ...
                    boolean replacementThroughCollidedMover =
                            ...
                            && world.movement
                                    .battleNetRefusalBandSoftClearMoveAlly(
                                            oldReplacementBlocker);
                    world.causalTrace.event(world.cycle,
                            "combat.retarget-route-decision", unit.id(),
                            "previous", previous.id(),
                            "previous_x", previous.tileX(),
                            "previous_y", previous.tileY(),
                            "candidate", candidate.id(),
                            "path_length", keepPathn,
                            "path_initial",
                                    unit.battleNetPathInitialLength(),
                            "path_steps", unit.battleNetPathStepsTaken(),
                            "old_heading", oldReplacementHeading,
                            "approved_heading", approvedReplacementHeading,
                            "collision", unit.battleNetCollisionCounter(),
                            "refusals", unit.battleNetRefusals(),
                            "settled_melee", settledMeleeResidualRetarget,
                            "completed_band", completedSettledRefusalBand,
                            "head_free", settledResidualHeadFree,
                            "single_paid_long",
                                    singlePaidLongRouteHeadTransfer);
                    if (unit.pathLength() > 1
                            && previous.type().building()
                            && candidate.type().building()) {
                        int prevScore = replacementPreviousScore;
                        int candScore = replacementCandidateScore;
                        ...
                        if (completedSettledRefusalBand) {
                            ...
                            if (world.actionMoveWalked
                                ...
                                unit.replacePeekHeading(
                                        approvedReplacementHeading);
                                if (singlePaidLongRouteHeadTransfer
                                        && unit.pathLength() > 1) {
                                    // One cooperative refusal does not revoke
                                    // a diagonal already approved by a full
                                    // route. On the residual target switch,
                                    // retail consumes that old head and makes
                                    // the replacement's direct first face the
                                    // second byte (XHuman 12 slot 1489:
                                    // SE,SW at fixture 173). Java's fresh ray
                                    // begins at the old origin, so its second
                                    // byte still describes the pre-handoff
                                    // square; repair it from the projected
                                    // native origin.
                                    int projectedX = unit.tileX()
                                            + Direction.deltaX(
                                                    approvedReplacementHeading)
                                            * world.battleNetMovementStride(
                                                    unit);
                                    int projectedY = unit.tileY()
                                            + Direction.deltaY(
                                                    approvedReplacementHeading)
                                            * world.battleNetMovementStride(
                                                    unit);
                                    int replacementFace = World
                            ...
                            if (incumbentPlanBeforePaidRefillRetarget) {
                                // A route which was previously parked and
                                // refilled retains one native NewPath
                                // transaction against its incumbent before
                                // the boundary scan installs a replacement
                                // quarry. The accepted target pointer and goal
                                // change in this same callback, but the newly
                                // approved compass buffer survives behind
                                // them. XHuman 12 slot 1490 therefore redraws
                                // E,E,E,SE toward knight 1495, accepts tower
                                // 1483's hit offer, and still commits E on
                                // fixture 144. Planning after the target swap
                                // produced W and visibly reversed the grunt.
                                ...
                                boolean saturatedParkedRetarget =
                                        ...
                                        && unit.battleNetCollisionCounter()
                                                >= 5
                                        && unit.battleNetRefusals() > 0;
                                boolean acceptedSaturatedParkedRetarget =
                                        replacementAfterPaidBand
                                        && unit.battleNetCollisionCounter()
                                                >= 5
                                        && unit.battleNetRefusals() == 0;
                                if (acceptedSaturatedParkedRetarget) {
                                    // Once a saturated replacement route has
                                    // actually committed and drained, its next
                                    // Attack-construction target handoff sees
                                    // the marching line as a wall again. The
                                    // zero refusal count is the accepted-route
                                    // generation boundary: XHuman 12 slot
                                    // 1492 retains the three movers on y=39,
                                    // writes S,SE,SE,SW and consumes S on
                                    // fixture 175. Soft-clearing them rebuilt
                                    // a twenty-byte SW route and pulled the
                                    // grunt sideways out of the battle.
                                    world.planTowardsAfterAcceptedSaturatedPark(
                                            unit, chased);
                                } else if (saturatedParkedRetarget) {
                                    // Attack construction, not a completed
                                    // Move refusal band, owns this saturated
                                    // park. Its replacement target must see
                                    // the hard formation wall. Treating the
                                    // stale Attack delay as a paid cooperative
                                    // wake soft-cleared the line and selected
                                    // SW; native XHuman 12 slot 1492 walls
                                    // around it with NW,SW,S,SE,E,E and commits
                                    // NW on the same callback.
                                    world.planTowards(unit, chased, true);
                                } else {
                                    world.planTowardsAfterRefusalBand(
                                            unit, chased,
                                            (replacementOwnsFullRouteBuffer
                                                    && Direction.isDiagonal(
                                                            oldReplacementHeading))
                                                    || (keepPathn >= 19
                                                            && chased.type()
                                ...
                                if (replacementAfterPaidBand
                                        && !acceptedSaturatedParkedRetarget
                                        && settledResidualHeadFree
                                        // A refused head has lost approval.
                                        // The replacement ray owns this
                                        // callback even when the stale face
                                        // was diagonal (XHuman 12 slot 1489
                                        // at fixture 192). Collision-free
                                        // paid parks retain the old face.
                                        && unit.battleNetRefusals() == 0
                                        // This is RI20 buffer reuse. Compact
                                        // accepted routes own no surviving
                                        // tail, so the replacement quarry's
                                        // fresh head wins (XHuman 12 slot
                                        // 1492 at fixture 194).
                                        && replacementOwnsFullRouteBuffer
                                        && Direction.isDiagonal(
                                                oldReplacementHeading)
                            ...
                            if (singlePaidLongRouteHeadTransfer
                                    && unit.pathLength() > 0) {
                                // The target scan can run just beyond the
                                // completed-Move predicate even though the
                                // old full route has the same native
                                // ownership. Carry its paid diagonal into the
                                // ordinary settled replacement transaction.
                                unit.replacePeekHeading(
                                        approvedReplacementHeading);
                                if (unit.pathLength() > 1) {
                                    int projectedX = unit.tileX()
                            ...
                            if (unit.battleNetAttackRefusalRecoveryStage()
                                    == 3) {
                                // Attack construction has already paid the
                                // handoff into Move. Its replacement route is
                                // drawn with the same cooperative formation
                                // view as a completed refusal band. Using the
                                // ordinary hardened corridor can report no
                                // path through allied traffic and leave the
                                // attacker apparently frozen precisely when
                                // timer one grants its move (XHuman 12 slot
                                // 1476 / Java 124, fixture 127).
                                if (unit.pathLength() == 0) {
                                    // There is no approved parked face left to
                                    // harden. Retaining the marker here keeps
                                    // collision-three allies solid and can
                                    // turn native's twenty-byte wall route
                                    // into an empty Java path.
                    ...
                    int postConstructionWallFaceHeading =
                    ...
                    if (saturatedWallFacePairParked) {
                        if (world.actionMoveWalked) {
                            // The movement subsystem parked the shared
                            // buffer on this residual-settle callback. Native
                            // returns with Move-start/timer one; the refill is
                            // a distinct active-order visit.
                            return;
                        }
                        // The following callback restores only the opposite
                        // face byte, advances collision once more, and owns a
                        // complete Move refusal band. A cold wall search here
                        // lets crowded melee units slide through their own
                        // formation instead of waiting for its opening.
                        unit.setPath(new PathFinder.Path(
                                PathFinder.Result.FOUND,
                                new int[] {saturatedWallFacePairHeading}));
                    ...
                    boolean postConstructionWallFaceRetry =
                            postConstructionWallFaceHeading >= 0
                            && unit.chasing()
                            && chased != null && chased.isAlive()
                            && !world.targets.inAttackRange(unit, chased);
                    if (postConstructionWallFaceRetry) {
                        int collision = unit.battleNetCollisionCounter();
                        unit.setPath(new PathFinder.Path(
                                PathFinder.Result.FOUND,
                                new int[] {postConstructionWallFaceHeading}));
                        // setPath clears a one-byte route's low collision
                        // generation; the parked native cursor does not.
                        unit.setBattleNetCollisionCounter(collision);
                        unit.setPathGoal(chased.tileX(), chased.tileY());
                        unit.setRouteSpent(false);
                        unit.setWaitCycles(0);
                    } else if (postConstructionWallFaceHeading >= 0) {
                        unit.setBattleNetPostConstructionWallFaceHeading(-1);
                    }
                    boolean directRefusalRecoveryProbe =
                            ...
                            && unit.chasing();
                    if (postConstructionWallFaceRetry) {
                        // The retained face is consumed by Move below.
                    } else if (directRefusalRecoveryProbe) {
                        ...
                        unit.setBattleNetChaseEmptyRouteReplan(false);
                        if (!accepted) {
                            int refusalGeneration =
                                    unit.battleNetRefusals() + 1;
                            unit.setBattleNetRefusals(refusalGeneration);
                            if (unit.battleNetSaturatedNearRecoveryFullRoute()) {
                                // The completed range-two collision generation
                                // grants one naked direct probe. A rejection
                                // hands the next Attack 3,2,1 completion to the
                                // complete route writer. XHuman 12 slot 1504
                                // rejects SW on fixture 159, then writes and
                                // commits its NW-led wall route on fixture 162.
                                // Farther one-step approaches do not carry this
                                // provenance and retain the repeated-direct
                                // ladder through refusal eight (slot 1501).
                                // Keep the saturated provenance until that
                                // complete route actually commits. Its first
                                // accepted byte is the native generation
                                // boundary; clearing it here left Java's
                                // refusal count attached to the whole route
                                // and contaminated the next RI20 redraw.
                            ...
                            if (refusalGeneration >= 8) {
                                // Direct probes are still refusals. Native
                                // carries their generation across each quiet
                                // Attack 3,2,1 retry; the eighth writes a
                                // complete replacement route and owns Move's
                                // fifteen-count band before trying its first
                                // heading. XHuman 12 slot 1501 / Java 99
                                // begins with generation one after its paid SW
                                // residual, retries six times, then records
                                // the full NE-led wall route at fixture 125.
                                // Leaving the count frozen at one repeated
                                // active-order draws forever, reassigned the
                                // next melee roll, and made the attacker look
                                // permanently stuck beside the fight.
                                world.planTowardsAfterRefusalBand(
                                        unit, retryTarget);
                                unit.setBattleNetDirectRefusalRecoveryProbe(
                                        false);
                                // A dedicated marker carries the retained
                                // replacement through the paid band. Reusing
                                // DirectRefusalRecoveryProbe changes live
                                // movement predicates, while projecting the
                                // collision-one nibble into Java occupancy
                                // makes an allied mover incorrectly solid.
                    ...
                    boolean saturatedOneStepNearApproach =
                            unit.battleNetCollisionCounter() >= 5
                            && unit.battleNetRefusals() >= 2
                            && unit.target() != null
                            && Math.max(
                    ...
                    boolean paidOneStepRefusalApproachSettled =
                            ...
                            && (unit.order() == Unit.Order.ATTACK
                                    || unit.order() == Unit.Order.ATTACK_MOVE);
                    boolean acceptedSaturatedOneStepApproachSettled =
                            world.actionMoveWalked
                            && unit.stepDrained() && !unit.isMoving()
                            && unit.routeSpent()
                            && unit.battleNetPathStepsTaken() == 1
                            ...
                            && (unit.order() == Unit.Order.ATTACK
                                    || unit.order() == Unit.Order.ATTACK_MOVE);
                    if (acceptedSaturatedOneStepApproachSettled) {
                        // A wall-prefix byte accepted at saturation owns the
                        // same settle -> active-order Still -> Attack 3,2,1
                        // handoff as an admitted hard-refusal byte. Native
                        // clears collision 0x60 and parks RI20 in this very
                        // callback. Refilling immediately made XHuman 12 slot
                        // 1482 refuse NW on fixture 194 and escape N on 195,
                        // while retail remains engaged and retries its direct
                        // SW face after construction.
                        Unit retryTarget = unit.target();
                        world.causalTrace.event(world.cycle,
                                "path.accepted-saturated-one-step-settle",
                                unit.id(), "target", retryTarget.id(),
                                "collision",
                                        unit.battleNetCollisionCounter(),
                    ...
                    if (paidOneStepRefusalApproachSettled) {
                        ...
                        // exhausted cursor, pays active-order Still, and enters
                        // Attack construction 3,2,1 before granting Move one
                        ...
                        // at 105. Reusing the ordinary stage-five refusal loop
                        // also preserves its idle draw and bounded reprobe
                        // cadence for every crowded melee approach.
                        Unit retryTarget = unit.target();
                        world.causalTrace.event(world.cycle,
                        ...
                        unit.setBattleNetChaseEmptyRouteReplan(false);
                        unit.setBattleNetDirectRefusalRecoveryProbe(true);
                        if (saturatedOneStepNearApproach) {
                            // At the range-two edge a saturated one-byte
                            // approach has completed its native collision
                            // generation. Retail clears the nibble and enters
                            // Attack 3,2,1 with route index twenty; it does not
                            // free-compass around the quarry in the settle
                            // callback. XHuman 12 slot 1504 is the cardinal
                            // witness. Farther saturated cardinal probes keep
                            // their immediate refill behavior.
                            unit.setBattleNetCollisionCounter(0);
                            unit.setBattleNetSaturatedNearRecoveryFullRoute(
                                    true);
                        }
                    ...
                    if (saturatedDiagonalRouteTerminator) {
                        // A compact face admitted at the top of the saturated
                        // ladder is the surviving head of a failed shared
                        // wall buffer, not the completed twenty-byte route
                        // handled below. Its residual parks that buffer and
                        // pays one more complete Move band while retaining
                        // pressure. XHuman 12 native slot 1506 carries the
                        // four-byte NW,SW,S,SE buffer behind Java's one-byte
                        // surrogate: collision eight becomes nine at fixture
                        // 184, Move counts 15..1, and the fresh route commits
                        // only at 199. Sending the compact form through
                        // active-order Still cleared its pressure, consumed an
                        // idle RNG draw, and let it step SE eleven fixtures
                        // early.
                        boolean compactPaidTerminatorBand =
                                (unit.battleNetPathInitialLength() == 1
                                        || (unit.battleNetPathInitialLength()
                                                        <= 4
                                                && unit.battleNetPathStepsTaken()
                        ...
                        // pays its idle draw, and enters Attack construction
                        // 3,2,1. Refilling here made the surrounded attacker
                        // slide through its line.
                        int collision =
                                unit.battleNetCollisionCounter() + 1;
                        unit.setBattleNetCollisionCounter(
                                collision > 14 ? 0 : collision);
                    ...
                    boolean paidCollisionFourImmediateRefill =
                            ...
                            && unit.battleNetCollisionCounter() == 4
                            && unit.battleNetRefusals() == 0;
                    boolean sharedTwoStepWallTerminator =
                            world.actionMoveWalked
                            && unit.stepDrained() && !unit.isMoving()
                            && unit.routeSpent()
                            && unit.battleNetCollisionCounter() == 0
                            ...
                            && unit.target() != null
                            && !World.battleNetRangedChaseUnit(unit);
                    if (sharedTwoStepWallTerminator) {
                        // Both wall probes wrote into one native route buffer.
                        // The surviving compass byte is the whole live route;
                        // its residual-settle visit therefore exposes cursor
                        // twenty before the next callback may redraw. Without
                        // this park, a fighter consumes the replacement route
                        // on the same visit and visibly jumps ahead of its
                        // crowded formation (XHuman 12 slot 1507, c181/182).
                        unit.clearPath();
                        unit.setRouteSpent(false);
                        unit.setStepDrained(false);
                        unit.setWaitCycles(0);
                    ...
                    if (world.actionMoveWalked
                            ...
                            && !(unit.routeSpent()
                                    ...
                                    && !Direction.isDiagonal(
                                            unit.lastStepHeading()))
                            // Collision generation four with no hard-refusal
                            // history is already-paid formation pressure. Its
                            // exhausted diagonal may refill and commit on the
                            // settle callback; generation five plus sticky
                            // refusals remains the saturated park above.
                            // XHuman 12 slot 1482 / Java 118 takes native NE
                            // on fixture 127 instead of waiting and drawing a
                            // stale north route on 128.
                    ...
                    boolean collidedResidualRefill =
                            (unit.battleNetChaseEmptyRouteReplan()
                                    || paidCollisionFourImmediateRefill)
                            && unit.battleNetCollisionCounter() > 0;
                    boolean postSaturatedTerminatorConstruction =
                            unit.battleNetSaturatedTerminatorRoutePending()
                            && unit.battleNetCollisionCounter() == 0
                            && unit.battleNetRefusals() == 0
                            && unit.battleNetChaseEmptyRouteReplan();
                    boolean completedAcceptedSingleRefusalResidual =
                            world.actionMoveWalked
                            && unit.stepDrained() && !unit.isMoving()
                            && unit.routeSpent()
                            && unit.battleNetPathStepsTaken() == 1
                            && unit.battleNetCollisionCounter() == 1
                            && unit.battleNetRefusals() > 0
                            && unit.battleNetAcceptedSingleRefusalCollisionCarry()
                            && unit.battleNetAttackRefusalRecoveryStage() == 0
                            && unit.type() != null
                            && unit.type().maxAttackRange() <= 1
                            && !World.battleNetRangedChaseUnit(unit);
                    boolean saturatedRecoveryRoutePark =
                            ...
                            && unit.type().maxAttackRange() <= 1
                            && !World.battleNetRangedChaseUnit(unit);
                    int collisionBeforeResidualRefill =
                            unit.battleNetCollisionCounter();
                    unit.setRouteSpent(false);
                    unit.clearPath();
                    if (candidate != null) {
                    ...
                    } else if (saturatedRecoveryRoutePark) {
                        // The full route accepted after a saturated direct
                        // probe retains the formation as a hard wall through
                        // its RI20 redraw. Native therefore sees the standing
                        // grunt immediately south as occupied and writes the
                        // two-face SE,SW buffer for XHuman 12 slot 1504 at
                        // fixture 195. An ordinary refill soft-cleared that
                        // body, wrote S,S, and entered another refusal band.
                        world.planTowardsAfterAcceptedSaturatedPark(
                                unit, chased);
                        unit.setBattleNetSaturatedNearRecoveryFullRoute(false);
                    ...
                    } else if (postSaturatedTerminatorConstruction) {
                        // The active-order callback cleared the completed
                        // diagonal collision generation, but its following
                        // Attack 3,2,1 route draw still owns the paid-band
                        // view of moving allies. Native XHuman 12 slot 1495
                        // therefore writes SE,E through the departing grunt
                        // on (31,40); the ordinary hardened view wrote SE,SE,
                        // which later refused and stalled the formation.
                        world.planTowardsAfterRefusalBand(unit, chased);
                    } else if (completedAcceptedSingleRefusalResidual) {
                        // A successful one-byte refill did not erase the
                        // collision generation which admitted it. When that
                        // byte settles, NewPath still sees the paid formation
                        // view: moving allies are writable route squares, then
                        // the live first-square probe enters the next
                        // cooperative generation. XHuman 12 slot 1495 writes a
                        // complete N-led route and changes 0x10 -> 0x20 with
                        // Move/15 on fixture 174. The ordinary hardened search
                        // returned no route and incorrectly sent the unit
                        // through active-order Still, consuming an RNG draw and
                        // visibly freezing it beside the battle.
                        unit.setBattleNetAcceptedSingleRefusalCollisionCarry(
                                false);
                        world.causalTrace.event(world.cycle,
                                "path.accepted-single-refusal-paid-refill",
                                unit.id(), "target", chased.id(),
                                "collision",
                                        unit.battleNetCollisionCounter(),
                                "refusals", unit.battleNetRefusals(),
                                "heading", unit.lastStepHeading());
                        world.planTowardsAfterAcceptedSingleRefusal(
                                unit, chased);
                    } else if (paidCollisionFourImmediateRefill) {
                        // Collision generation four with no refusal history
                        // has already paid the cooperative formation band.
                        // Its residual-settle refill therefore uses the same
                        // moving-ally view as a completed refusal, while
                        // retaining the first wall face written into BNE's
                        // route buffer. XHuman 12 slot 1482 then writes the
                        // native NE,SE,E,... route and commits NE on fixture
                        // 127 instead of taking the cold search's north wall.
                        world.planTowardsAfterRefusalBand(
                                unit, chased, true);
                    } else {
                    ...
                    if (collisionBeforeResidualRefill == 1
                            && unit.battleNetRefusals() > 0
                            && unit.battleNetAttackRefusalRecoveryStage() == 0
                            && unit.type() != null
                            && unit.type().maxAttackRange() <= 1
                            ...
                            && unit.pathLength() == 1
                            && unit.battleNetCollisionCounter() == 0) {
                        // A refused residual refill starts a fresh collision
                        // generation, but a byte which commits keeps the paid
                        // generation while its pixels drain. Restore only after
                        // an accepted tile commit so blocked one-byte refills
                        // still begin their new refusal at generation one.
                        carryRefusalCollisionAfterSingleStep =
                                collisionBeforeResidualRefill;
                    }
                    ...
                    boolean acceptedSaturatedResidualFaceRoute =
                            unit.battleNetSaturatedResidualFaceRetry()
                            && unit.pathLength() == 1
                            && unit.battleNetCollisionCounter() >= 5
                            && unit.battleNetRefusals() == 0;
                    if (!acceptedSaturatedResidualFaceRoute) {
                        unit.setBattleNetSaturatedResidualFaceRetry(false);
                    }
                    unit.setBattleNetSaturatedTerminatorRoutePending(false);
            ...
            if (unit.battleNetAttackRefusalRecoveryStage() == 3) {
                ...
                boolean changedDuringPaidConstruction =
            ...
            boolean deferSettledRetargetSync = world.actionMoveWalked
                    ...
                    && unit.type() != null
                    && PudUnitTypes.code(unit.type().ident()) == 0;
            Unit postConstructionTarget = unit.target();
            boolean emptyPostConstructionUnitWallProbe =
                    unit.battleNetAttackRefusalRecoveryStage() == 6
                    && !unit.battleNetDirectRefusalRecoveryProbe()
                    && unit.pathLength() == 0
                    && unit.chasing()
                    && postConstructionTarget != null
                    && postConstructionTarget.isAlive()
                    && postConstructionTarget.type() != null
                    && !postConstructionTarget.type().building()
                    && !world.targets.inAttackRange(
                            unit, postConstructionTarget)
                    && unit.type() != null
                    && unit.type().maxAttackRange() <= 1
                    && !World.battleNetRangedChaseUnit(unit)
                    && (unit.order() == Unit.Order.ATTACK
                            || unit.order() == Unit.Order.ATTACK_MOVE);
            if (emptyPostConstructionUnitWallProbe) {
                int direct = World.battleNetFirstBresenhamHeading(
                        unit.tileX(), unit.tileY(),
                        postConstructionTarget.tileX(),
                        postConstructionTarget.tileY());
                int stride = world.battleNetMovementStride(unit);
                int directX = unit.tileX()
                        + Direction.deltaX(direct) * stride;
                int directY = unit.tileY()
                        + Direction.deltaY(direct) * stride;
                int face = Math.floorMod(direct - 1, Direction.COUNT);
                int faceX = unit.tileX()
                        + Direction.deltaX(face) * stride;
                int faceY = unit.tileY()
                        + Direction.deltaY(face) * stride;
                int currentDistance = Math.max(
                        Math.abs(unit.tileX()
                                - postConstructionTarget.tileX()),
                        Math.abs(unit.tileY()
                                - postConstructionTarget.tileY()));
                int faceDistance = Math.max(
                        Math.abs(faceX - postConstructionTarget.tileX()),
                        Math.abs(faceY - postConstructionTarget.tileY()));
                if (Direction.isDiagonal(direct)
                        && !world.canEnter(unit, directX, directY)
                        && world.canEnter(unit, faceX, faceY)
                        && faceDistance <= currentDistance) {
                    // The mobile-quarry diagonal writer tries exactly one
                    // negative-rotation cardinal face after Attack
                    // construction. It retains the blocked diagonal byte;
                    // XHuman 12 slot 1434 therefore stores N,NE when north
                    // opens at fixture 166. This is not the building-goal
                    // writer: ogre 1356 keeps its empty route against a guard
                    // tower at fixture 91. Nor is it a compass scan: slot
                    // 1516 and grunt 1456 keep their cardinal writers parked
                    // even though a later compass square is free.
                    unit.setPath(new PathFinder.Path(
                            PathFinder.Result.FOUND,
                            new int[] {direct, face}));
                    unit.setPathGoal(postConstructionTarget.tileX(),
                            postConstructionTarget.tileY());
                    unit.setRouteSpent(false);
                    unit.setWaitCycles(0);
                    unit.setBattleNetOrderDelay(0);
                    unit.setBattleNetChaseEmptyRouteReplan(false);
                    world.causalTrace.event(world.cycle,
                            "path.post-construction-unit-negative-face",
                            unit.id(), "target", postConstructionTarget.id(),
                            "direct", direct, "face", face);
                }
            }
            if (unit.battleNetAttackRefusalRecoveryStage() == 6
                ...
                int faceY = unit.tileY()
                        + Direction.deltaY(face) * stride;
                if (Direction.isDiagonal(direct)
                        && unit.peekHeading() == face
                        && world.battleNetSharedDyingMovingAllyFace(
                                unit, postConstructionTarget, faceX, faceY)) {
                    unit.setBattleNetPostConstructionWallFaceHeading(face);
                }
            }
            ...
            if (committedChaseHeading
                    && carryRefusalCollisionAfterSingleStep > 0
                    && unit.battleNetCollisionCounter() == 0) {
                unit.setBattleNetCollisionCounter(
                        carryRefusalCollisionAfterSingleStep);
                unit.setBattleNetAcceptedSingleRefusalCollisionCarry(true);
                world.causalTrace.event(world.cycle,
                        "path.accepted-single-refusal-collision-carry",
                        unit.id(), "collision",
                                carryRefusalCollisionAfterSingleStep,
                        "refusals", unit.battleNetRefusals(),
                        "heading", unit.lastStepHeading());
            ...
            if (exhaustedMeleeApproach && world.actionMoveWalked) {
                if (pathLengthBeforeChaseWalk == 0
                    ...
                    // A cursor already exhausted before the final pixels
                    // settle enters active-order Still and Attack construction
                    // on this callback. The authenticated async trace places
                    // slot 1434's 0040AD58 draw immediately before its
                    // Attack-2539/3 state at fixture 127. A unit which still
                    // held a cached heading before settlement remains the
                    // deferred Still case below (slot 1381 / 219).
                    unit.setBattleNetResidualEmptyApproachIdlePending(false);
                    unit.setWaitCycles(0);
                    unit.setBattleNetOrderDelay(0);
                    if (rearmBattleNetHardRefusalAttack(unit)) {
                ...
                // The visit which drains an approach's final pixels owns only
                // the route-index-20 park.  Active-order Still (and therefore
                // its asynchronous idle draw) is the following scheduler
                // callback.  Folding both visits together advanced every
                // later idle, missile and damage consumer by one draw whenever
                // a long melee ray ended at an unreachable goal. XHuman 12's
                // ogre 1381 / Java 219 is the authenticated witness: fixture
                // 118 settles E without a draw; fixture 119 draws and enters
                // Attack construction 3,2,1.
                unit.setBattleNetResidualEmptyApproachIdlePending(true);
                unit.setWaitCycles(0);
                unit.setBattleNetOrderDelay(0);
                return;
            ...
            if (unit.animation().unbreakable() || committedChaseHeading) {
                // A successful NextPathElement commit owns the remainder of
                // this MoveToTarget callback even when ChonkCraft's display
                // animation happens to be on a breakable frame. Retail's
                // native Move program is unbreakable after the commit, so it
                // does not run CheckForTargetInRange from the newly claimed
                // logical tile. The scan remains due and runs when that
                // stride's pixels finish. In a crowded line this matters:
                // XHuman 12 slot 1481 commits NE at fixture 132 while the
                // footman and knight candidates still tie at the old anchor,
                // retains its target and route through the residual, then
                // selects the knight from the settled tile at fixture 148.
                // Scanning after Java's commit selected the knight sixteen
                // visits early, erased seventeen approved route bytes and
                // sent the grunt through an unrelated free detour.
                return;
            }
            if (underWay) {
    ...
    /**
     * Defers table-0x27's first draw when a footman's completed chase residual
     * names a different in-range quarry.
     *
     * <p>Retail enters cold Attack construction first and calls
     * {@code FUN_004234b0} only when OP0 is reached. The same handoff can arrive
     * through a retained route, PF_REACHED, or an exhausted multi-step route;
     * keeping this decision in one gate prevents those equivalent native states
     * from charging the synchronized stream on different visits. XHuman 4
     * footman 1510 is the binary witness: retarget on fixture 82, Attack timers
     * 3,2,1 through 84, SyncRand on 85.</p>
     */
    private boolean deferBattleNetResidualRetargetSyncRand(
            Unit unit, Unit target, Unit targetBeforeWalk) {
        if (target == targetBeforeWalk || unit.type() == null
                || PudUnitTypes.code(unit.type().ident()) != 0
                || !unit.battleNetPendingMeleeSyncRand()) {
            return false;
        }
        if (World.BNE_PEND_TRACE) {
            System.err.printf("JBNEMELEESYNC event=defer-retarget cycle=%d "
                    ...
                    world.cycle, unit.id(),
                    target == null ? -1 : target.id(),
                    targetBeforeWalk == null ? -1 : targetBeforeWalk.id());
        }
        world.armBattleNetAttackStart(unit);
        unit.setBattleNetOrderDelay(2);
        world.turnToTarget(unit, target, 0, 0);
    ...
    void stepAttackMove(Unit unit) {
        ...
        boolean battleNetDeadGoalBoundary = world.battleNetSequence != null
                ...
                && ((sequenceBoundaryGoal != null
                        ...
                        || (sequenceBoundaryGoal == null && unit.fighting()));
        if (!battleNetDeadGoalBoundary
                && unit.animation().unbreakable() && world.isSwinging(unit)) {
            world.finishSwing(unit);
            if (unit.animation().unbreakable()) {
                return;
            }
        ...
        unit.setStepDrained(false);
        boolean deferDrop = !battleNetDeadGoalBoundary && ((unit.chasing()
    ...
    boolean stepBattleNetAttackSequence(Unit unit) {
        ...
        if (tick.actionMarker()
            ...
            if (tgt != null && tgt.isAlive()
                ...
                // A free-scan replacement owns a fresh native OP0 period even
                // when the discarded quarry's wall-clock cadence has partly
                // drained during the chase. XHuman 12 axe 1529 reaches its
                // replacement target with 44 left, but BNE restarts the full
                // 63-count Attack@887 hold. Ordinary approach resumes still
                // consume only the unspent cadence, preserving their timing.
                int hold = unit.battleNetRangedFreeScanHoldActive()
        ...
        boolean committedRecoveryTail = !tick.actionMarker()
                && unit.fighting() && !unit.isMoving()
                && (!unit.battleNetStationaryAttack()
                        || sequenceTarget == null
                        || !sequenceTarget.isAlive())
                && !settledInRange
                // Attack-start is construction/OP0 and can still select a
                // replacement. Once the cursor has entered the body, the
                // current swing owns every remaining non-marker visit even
                // if Java's parallel presentation becomes breakable early.
                // XHuman 12 knight 1446 keeps Attack tail 1945/3,2,1 after
                // its quarry dies and only chooses the hit-back target on the
                // following OP0 fixture.
        ...
        if (!tick.actionMarker()) {
            ...
            boolean rangedStationaryCommittedHold = rangedOp0
                    ...
                    && unit.battleNetAttackResumeHoldActive()
                    && attackStart >= 0 && offset == attackStart;
            return deferMeleeRetarget || chaseDecision || committedRecoveryTail
    ...
    boolean rearmBattleNetHardRefusalAttack(Unit unit) {
        ...
        // Native reaches this boundary through active-order idle dispatch.
        // That dispatch owns one asynchronous draw (and its rare facing
        // change) before Attack construction begins. Missing the debit did
        // not merely move critters differently: it reassigned later damage
        // and projectile draws to the wrong combatants.
        world.idle.advanceBattleNetActiveOrderIdleRandom(unit);
        return armBattleNetHardRefusalAttackConstruction(unit);
    }

    /** Opens hard-refusal Attack construction without an active-order draw. */
    private boolean armBattleNetHardRefusalAttackConstruction(Unit unit) {
        int attackStart = world.idle.battleNetSequenceStart(unit,
                BattleNetSequence.ATTACK_ANIMATION);
        if (attackStart < 0) {
            return false;
    ...
    private boolean stepBattleNetAttackRefusalRecovery(Unit unit) {
        ...
        if (stage == 4) {
            ...
            if (unit.pathLength() == BattleNetPathFinder.MAX_PATH
                ...
                // A direct-recovery replacement is a retained cooperative
                // route, not the empty buffer owned by an eighth hard
                // refusal. Native parks its twenty-byte cursor when timer one
                // wakes (collision one -> two), then replans and consumes the
                // new head on the following callback. Sending this state
                // through Attack construction added three visible frozen
                // visits and missed both neighbours vacating the formation.
                unit.clearPath();
                unit.setRouteSpent(false);
