...
final class BattleNetMovementSystem {
    ...
    boolean promoteQueuedPlayerMoveAfterLeftover(Unit unit) {
        ...
        unit.setBattleNetOrderDelay(world.battleNetSequence == null ? 2
                : Math.max(0, playerCommandWaits(unit)[0] - 1));
        unit.setActionBeforeQueued(null);
        return true;
    }
    ...
    boolean alreadyTouchingBlockedDest(Unit unit, int toX, int toY) {
        ...
        if (toX == unit.tileX() && toY == unit.tileY()) {
            // The unit's own occupancy does not reject a new Move. Human 7's
            // peasant queues a click on its current tile at 75, constructs
            // Move at 79, and completes it at 82. Swallowing the click kept
            // the old idle callback alive instead of replacing the order.
            return false;
        }
    ...
    boolean orderCommandMove(Unit unit, int toX, int toY) {
        ...
        if (world.battleNetSequence != null) {
            // Move constructor 0x4363b0 caps the current animation timer at
            // 10 for a person and 40 for a computer before queueing Move.
            // A juggernaught's 127-count firing wait used to ignore this
            // click: the native c150 command exposes timer 9 immediately.
            unit.setBattleNetAnimationTimer(Math.min(unit.battleNetAnimationTimer(),
                    world.isPerson(unit.player()) ? 10 : 40));
        }
        ...
        // three cycles early.
        // That compensation belongs only to native Still's first opcode.
        // XOrc 11 catapult 1495 is already on shared Still at its click;
        // charging the sprite's wait there delayed its first stride and the
        // nearby worker's replacement route by three visits.
    ...
    /** A behavior-one melee fighter returning home on its native Move program. */
    private boolean battleNetLandRegroupMove(Unit unit) {
        return world.battleNetSequence != null
                && unit.order() == Unit.Order.MOVE
                && !unit.battleNetPlayerCommandMove()
    ...
    void stepMoveOrder(Unit unit) {
        boolean playerSeaStride = world.battleNetSequence != null
                && unit.order() == Unit.Order.MOVE && unit.battleNetPlayerCommandMove()
                && unit.type().seaUnit() && unit.isMoving()
                && unit.battleNetMovePaceOffset() >= 0;
        moveOrderDepth++;
        try {
            stepMoveOrderVisit(unit);
            if (playerSeaStride && unit.order() == Unit.Order.MOVE
                    && !unit.isMoving() && unit.pathLength() == 0
                    && !unit.hasQueuedOrders()
                    && unit.tileX() == unit.orderTargetX() && unit.tileY() == unit.orderTargetY()) {
                // The final native Move callback lands and constructs Still
                // in one visit: XHuman 7's submarine at fixture 206.
    ...
    private void stepMoveOrderVisit(Unit unit) {
        if (unit.battleNetOrderDelay() > 0) {
            // The queued Move keeps the current Still constructor until its
            // first movement marker. Human 13 ogre 1519 counts 3,2,1 on
            // 249..251. A logical delay without the raw timer used to leave
            // neighbouring route queries seeing a stationary body.
            if (battleNetLandRegroupMove(unit)
                    && unit.battleNetAnimationTimer()
            ...
            syncBattleNetAttackRefusalTimer(unit);
        ...
        if (!unit.isMoving() && !isStepping(unit) && unit.pathLength() == 0) {
            ...
            switch (path.result()) {
                case REACHED -> {
                    if (world.battleNetSequence != null && unit.battleNetPlayerCommandMove()) {
                        // A new Move to the current tile still completes via
                        // Still's constructor. Human 7's peasant completes at
                        // 82 and first idles at 85, not the following visit.
                        finishBattleNetMoveAtTarget(unit);
                        return;
                    }
                    resetDisplacement(unit);
                ...
                default -> {
                    ...
                    if (unit.pathLength() > 0
                            && !unit.battleNetBorrowedMoveForStep()
                            && !plainMoveRefusalReplacement
                            // A recursive same-visit refill has already asked
                            // the pathfinder for the native replacement route.
                            // Rewriting that route as a direct line changed
                            // XHuman 12 axethrower 1359's c382 first step from
                            // north-east to east. Only the outer Move visit
                            // owns this refusal-replacement convention.
                            && moveOrderDepth == 1
    ...
    private void battleNetEmptyRouteStillAndDispatch(Unit unit) {
        ...
        unit.clearPath();
        unit.setBattleNetPlayerCommandMove(false);
        // An empty escape route still restores the resource order it
        // interrupted. Dispatching ordinary idle reassigned Human 8 worker
        // 1533 to a fresh job at 329 and abandoned its loaded return.
        if (world.harvest.restartBattleNetStrandedResourceHitFlee(unit)
                || world.harvest.finishBattleNetResourceHitFlee(unit)) {
            return;
        }
    ...
    private void finishBattleNetMoveAtTarget(Unit unit, int stillTimer) {
        ...
        if (world.harvest.finishBattleNetResourceHitFlee(unit)) {
            return;
        }
        if (unit.savedOrder() == Unit.Order.STILL) {
            unit.takeSavedOrder();
     ...
     *
     * <p>The point-route wall follower reads the unit's current action body
     * before consulting the packed collision nibble. Java keeps some
     * refusal generations outside that byte, so the narrow action-23 redraw
     * after a fully paid forest prefix must be able to ask the first question
     * independently. In XHuman 12, axethrower 1359 and peon 1376 are both on
     * their Move bodies at fixture 333 and native clears their occupied cells;
     * the three settled harvest neighbors remain solid.</p>
     */
    boolean battleNetCurrentMoveBody(Unit candidate) {
        if (candidate == null || !candidate.isMoving()) {
            return false;
        }
        if (world.battleNetSequence == null) {
    ...
    /** A command must also drain a stride whose native cursor is held by its residual clock. */
    boolean battleNetCommandMoveBody(Unit unit) {
        // A player Move can use the presentation pace while its primary
        // cursor still names Still. XHuman 9's slot 1420 starts that stride
        // at 100; Attack at 111 cannot spend another heading before it lands.
        return battleNetCurrentMoveBody(unit)
                || unit.isMoving() && world.battleNetMoveAnimation(unit)
                || battleNetCommandRefusalBody(unit);
    }

    /** A blocked point order retains its paid Move callback until the timer expires. */
    boolean battleNetCommandRefusalBody(Unit unit) {
        return world.battleNetSequence != null && !unit.isMoving()
                && (unit.battleNetOrderDelay() > 0 || unit.queuedReplacementPending()
                        || (unit.battleNetPlayerCommandMove()
                                && unit.battleNetCollisionCounter() > 0))
                && unit.battleNetAnimationTimer() > 0
    ...
    private Unit battleNetDrainingDepotEntryBlocker(
        ...
        for (Unit blocker : world.unitsSnapshot()) {
            ...
            // body without making collision- or refusal-marked allies
            // generally passable. XHuman 12 slot 1555 independently carries
            // Java's collision generation two while its north entry pixels
            // drain; slot 1550 still commits north-east onto the same hall
            // point at fixture 396.
            return blocker;
        }
        return null;
    }
     ...
     */
    boolean battleNetEmptyRouteRefillsImmediately(Unit unit) {
    ...
    private void armBattleNetAttackRefusalMove(Unit unit) {
        // A player's point Move owns the same refusal program. XOrc 11
        // grunt 1488 pays Move 15..1 at 95..109; Stop must see that body
        // and construct Still on 110. A delay with no native cursor let
        // Stop dispatch idle immediately and steal a later damage draw.
        boolean playerPointMove = unit != null
                && unit.order() == Unit.Order.MOVE
                && unit.battleNetPlayerCommandMove();
        if (world.battleNetSequence == null
                || unit == null
                || (!playerPointMove && (unit.target() == null
                        || !(unit.order() == Unit.Order.ATTACK
                                || unit.order() == Unit.Order.ATTACK_MOVE
                                || unit.chasing())))) {
            return;
        ...
        boolean stagedMobileResidualRefill =
                stagedBattleNetMobileResidualRefill(
                        unit, settledMultiResidual);
        ...
        boolean retainedAttackTail = paidAttackTailGenerationPark(unit);
        // A constructor-paid probe can expose Move's first body opcode
        // before its new route is refused. It still owns a fresh full band:
        // XHuman 12 grunt 1457 writes Move 15 at 344, then steps on 359.
        boolean freshPaidRouteProbe = unit.battleNetAttackRefusalRecoveryStage() == 6
                && unit.battleNetChaseEmptyRouteReplan()
                && unit.battleNetPathStepsTaken() == 0;
        if (executingMove && !playerPointMove && !freshPaidRouteProbe
                && !refusedOnActionMarker
        ...
        unit.setBattleNetAnimationTimer(
                stagedMobileResidualRefill && !retainedBuildingResidual
                        ? 1 : 15);
        unit.setBattleNetChaseStepReady(false);
        if (retainedAttackTail) {
            // A completed Attack-tail pursuit retains its cached route when
            // a moving ally blocks the next byte. Native 0x437a25 writes 15
            // for XHuman 4 footman 1498 at 331; the east step belongs to 346.
            // Keeping the live Move cursor let the diagonal-detour shortcut
            // cancel this wait and step northeast on 332 instead.
    ...
    private boolean stagedBattleNetMobileResidualRefill(
        ...
        boolean retainedFullBufferTail = settledMultiResidual
        ...
        if (mayDecide) {
            ...
            if (!canTakeStep) {
                ...
                if (paidRecoveryResidualTailPark) {
                    ...
                    // generation. A live moving quarry, or a tail which
                    // already carries collision ownership, keeps this
                    // transaction in Move. A clean behavior-two stationary-
                    // quarry tail instead returns through active-order Still
                    // and Attack construction (XHuman 12 slot 1453 at
                    // fixture 209); behavior one retains the route for a
                    // complete regroup band below.
                    boolean movingQuarry = unit.target().isMoving()
                            && unit.target().type() != null
                            && !unit.target().type().building();
                    boolean retainedRoutePressure = movingQuarry
                            || (unit.battleNetCollisionCounter() > 0
                                    && unit.battleNetRefusals() == 0);
                    boolean regroupTailRefusalBand = !movingQuarry
                            && !retainedRoutePressure
                            && unit.battleNetAiBehavior() == 1;
                    world.causalTrace.event(world.cycle,
                            ...
                            "collision", unit.battleNetCollisionCounter(),
                            "refusals", unit.battleNetRefusals(),
                            "moving_target", unit.target().isMoving(),
                            "regroup_band", regroupTailRefusalBand);
                    unit.setBattleNetPaidRefusalRecoveryApproach(false);
                    if (regroupTailRefusalBand) {
                        // A behaviour-one regroup fighter retains the paid
                        // recovery buffer when its first committed heading
                        // settles against the next occupied byte. Native
                        // starts collision generation one and Move 15..1 on
                        // that same settle callback. XHuman 12 slot 1504
                        // keeps S,SW at route index one on fixture 356;
                        // behaviour-two slot 1453 is the opposing witness,
                        // parking its comparable SE,NE tail on fixture 208
                        // before returning through Attack construction.
                        int collision = unit.battleNetCollisionCounter() + 1;
                        unit.setBattleNetCollisionCounter(
                                collision > 14 ? 0 : collision);
                        unit.setRouteSpent(false);
                    ...
                    unit.setBattleNetPaidCollisionTailRefill(
                            !movingQuarry && retainedRoutePressure);
                    if (movingQuarry) {
                        int collision = unit.battleNetCollisionCounter() + 1;
                        unit.setBattleNetCollisionCounter(
                                collision > 14 ? 0 : collision);
                    } else if (!retainedRoutePressure) {
                ...
                // 0x437ab4 checks the incremented collision count before
                // inspecting the blocker's next destination. Eight through
                // fourteen keep the full fifteen-count wait even when that
                // blocker has no route byte left (XHuman 12 grunt 1457 at
                // 188 and 203). The Attack caller must retain this owner.
                if (unit.battleNetAttackRefusalRecoveryStage() == 6
                        && unit.battleNetCollisionCounter() >= 7
                        && unit.battleNetCollisionCounter() < 14
                        && unit.offeredTarget() == null
                        && stageSixBlocker != null
                        && world.isAllied(unit.player(), stageSixBlocker.player())) {
                    unit.setBattleNetCollisionCounter(
                            unit.battleNetCollisionCounter() + 1);
                    unit.clearPath();
                    unit.setRouteSpent(false);
                ...
                boolean duplicateCardinalStageSixProbe =
                        ...
                        && (paidStageSixCooperativeRoute
                                || (unit.battleNetAttackRefusalRecoveryStage() == 6
                                        ...
                                        && world.isAllied(unit.player(),
                                                stageSixBlocker.player())
                                        && world.combat.onBattleNetChaseMoveBody(
                                                stageSixBlocker)))
                        && unit.battleNetChaseEmptyRouteReplan()
                        && unit.pathLength() >= 1 && unit.pathLength() <= 2
                        && unit.battleNetPathStepsTaken() == 0
                        && unit.battleNetCollisionCounter() <= 1
                        && unit.battleNetRefusals() == 0
                        && !Direction.isDiagonal(heading)
                ...
                if (duplicateCardinalStageSixProbe) {
                    // Attack construction's final Move probe can receive E
                    // or the older duplicate E,E surrogate. Retail writes
                    // only direct E and parks its cursor at twenty:
                    ...
                    // Initialize this park once; later visits must increment
                    // its collision generation instead of resetting it to one.
                    unit.clearPath();
                    unit.setPath(new PathFinder.Path(
                            PathFinder.Result.FOUND,
                            new int[] {heading}));
                ...
                if (!goldApproach
                    ...
                    if (!canTakeStep) {
                        ...
                        unit.setBattleNetCollisionCounter(
                                counter > 14 ? 0 : counter);
                        boolean paidFullForestPrefixResidualPark =
                                unit.battleNetBorrowedMoveForStep()
                                && !unit.returningToDepot()
                                && unit.resourceUnit() == null
                                && unit.resourceTileX() >= 0
                                ...
                                && unit.battleNetWoodOrderY()
                                        == unit.resourceTileY();
                        if (paidFullForestPrefixResidualPark) {
                            // The second occupied probe closes the paid
                            // collision generation by parking the old route at
                            // index twenty. Action 23 owns the fresh NewPath
                            // call on its following visit; retaining another
                            // complete Move band here repeats the refusal and
                            // strands a saturated wood lane.
                            unit.setBattleNetRefusals(0);
                            unit.setBattleNetWoodRouteIndex20(true);
                            unit.setRouteSpent(false);
                            unit.setWaitCycles(0);
                        ...
                        boolean completedRetainedProgressPathn1Band =
                                unit.stepDrained()
                                && !unit.isMoving()
                                && unit.pathLength() == 1
                                && unit.target() != null
                                ...
                                && unit.battleNetRefusals() == 1
                                && !unit.battleNetMovingQuarryResidual();
                        // The first byte of a two-byte formation route can
                        // settle into one full cooperative band, leaving its
                        // second byte as the paid route tail. On that band's
                        // blocked wake native raises the collision generation
                        // and immediately starts the next Move 15..1 band;
                        // the current free-neighbour scan no longer owns the
                        // route. XHuman 12 slot 1489 retains SW and changes
                        // collision two to three on fixture 358. Parking the
                        // byte there made Java redraw and step west on 359.
                        boolean residualSettledPathn1 =
                                residualSettledPathn1Base
                                && !completedRetainedProgressPathn1Band
                                && !battleNetHasStrictlyCloserFreeNeighbour(
                                        unit, unit.target());
                        ...
                        if ((residualSettledPathn1
                                || (counter > 1
                                        && !retainedProgressPathn1))
                                && unit.pathLength() == 1
                                && unit.target() != null
                                && !World.battleNetRangedChaseUnit(unit)
                                && !unit.battleNetMovingQuarryResidual()
                        ...
                        if (multiResidualSecondRefuse) {
                            ...
                            boolean replanHold = unit
                                    ...
                                    && unit.pathLength() > 0
                                    && !World.battleNetRangedChaseUnit(unit);
                            // Human 8 attacker 1526 queues Attack after its
                            // first blocked SE probe at 363. Native Move pays
                            // fifteen visits, then Attack constructs for three;
                            // combining the waits consumes the command early.
                            boolean firstQueuedReplacementRefusal = replanHold
                                    && unit.battleNetPathStepsTaken() == 0
                                    && unit.battleNetTailWrapRouteTarget() != null
                                    && unit.battleNetTailWrapRouteTarget()
                                            != unit.target();
                            if (firstQueuedReplacementRefusal) {
                                unit.setBattleNetOrderDelay(14);
                ...
                if (!canTakeStep
                    ...
                    if (retainedCardinalPressureBand) {
                        ...
                        // this nearly-full cardinal face twice. Native advances
                        // its collision generation and pays the complete Move
                        // band while retaining the route buffer, so the unit
                        // resumes through the battle line instead of escaping
                        // diagonally from a cold replan. XHuman 12 slot 1517
                        // carries refusal two at fixture 147. Slot 1504 is the
                        // opposing one-refusal witness: it parks the buffer on
                        // fixture 388 and returns through active-order idle.
                        int collision = unit.battleNetCollisionCounter() + 1;
                        unit.setBattleNetCollisionCounter(
                                collision > 14 ? 0 : collision);
                        unit.setWaitCycles(0);
                    ...
                    if (!canTakeStep) {
                        ...
                        boolean firstSaturatedResidualProgressiveRefill =
                                ...
                                && !unit.target().type().building()
                                && !World.battleNetRangedChaseUnit(unit);
                        boolean paidCollisionSameFaceTailPark =
                                unit.stepDrained() && !unit.isMoving()
                                && unit.battleNetPathInitialLength()
                                        == BattleNetPathFinder.MAX_PATH
                                && unit.pathLength()
                                ...
                                && !unit.target().type().building()
                                && !World.battleNetRangedChaseUnit(unit);
                        boolean paidCardinalWallFacePark =
                                unit.stepDrained() && !unit.isMoving()
                                && unit.battleNetPathInitialLength()
                                        == BattleNetPathFinder.MAX_PATH
                                && unit.battleNetPathStepsTaken() >= 2
                        ...
                        if (paidCollisionSameFaceTailPark
                                && refusals == 1) {
                            // When the first accepted leg and the next blocked
                            // byte are the same diagonal, the full route still
                            // belongs to its paid collision-tail transaction.
                            // Native parks generation two and grants the next
                            // direct quarry ray. XHuman 12 slot 1490 repeats
                            // NE at fixture 374 and consumes direct SW on 375.
                            // Slot 1506's S -> NW transition at fixture 55 is
                            // the negative control and buys a complete band.
                            unit.setBattleNetPaidCollisionTailRefill(true);
                        }
                        if (paidCardinalWallFacePark && refusals == 1) {
                            // A full paid route which has already accepted a
                            // repeated cardinal run keeps that wall face when
                            // its next identical byte is refused. Native parks
                            // the old cursor, advances collision generation
                            // one to two, then gives the following NewPath call
                            // the retained-face writer. XHuman 12 slot 1517
                            // consumes two east bytes, parks the third on
                            // fixture 377, and writes the complete northeast
                            // wall route on 378. Treating the callback as a
                            // cold collided refill selects the north wall.
                ...
                if (!canTakeStep && !goldApproach
                    ...
                    if (woodAlly) {
                        ...
                        if (!canTakeStep) {
                            ...
                            boolean freshFarCardinalWoodRouteRefusal =
                                    unit.pathLength() > 1
                                    && unit.battleNetCollisionCounter() == 0
                                    && unit.battleNetRefusals() == 0
                                    && Direction.isDiagonal(
                                    ...
                                    && !walkedThisCycle;
                            boolean freshFarCardinalWoodRoutePark =
                                    freshFarCardinalWoodRouteRefusal
                                    && woodBlocker.battleNetRefusals() > 0;
                            if (freshFarCardinalWoodRoutePark) {
                                // A current-Move body whose own packed
                                // refusal generation is already sticky is no
                                // longer the cooperative owner of the cached
                                // forest ray. Native parks that stale cursor
                                // at RI20 and lets action 23 redraw against
                                // current occupancy on the following visit.
                                // XHuman 12 peon 1386 settles at (14,86) on
                                // fixture 396 with E covered by refusal-one
                                // axethrower 1359, then redraws and spends SE
                                // on 397. The same blocker at fixture 316 has
                                // no sticky refusal and retains E for a full
                                // Move band, providing the paired control.
                                unit.setBattleNetCollisionCounter(1);
                                unit.clearPath();
                                unit.setRouteSpent(false);
                                unit.setWaitCycles(0);
                            ...
                            int woodIndex = unit.resourceTileX()
                                    + unit.resourceTileY() * world.map.width();
                            if (unit.pathLength() == 1
                                    && unit.battleNetCollisionCounter() == 0
                                    && unit.battleNetRefusals() == 0
                                    && unit.battleNetGoldFreePrefixLength() == 3
                                    && unit.battleNetPathInitialLength() == 3
                                    && unit.battleNetPathStepsTaken() == 2
                                    && world.map.contains(
                                            unit.resourceTileX(),
                                            unit.resourceTileY())
                                    && world.battleNetClaimedWood.get(woodIndex)
                                            == woodBlocker) {
                                // The body refusing the last route byte also
                                // owns this tree. Native parks RI20 now, then
                                // action 23 replaces the claimed tree on the
                                // following visit rather than retrying its
                                // occupied approach forever.
                ...
                // A retired cold chase returns home with the same parked
                // collision generation as its combat route. Human 13 ogre
                // 1510 refuses cached N at 267, parks RI20 with collision
                // one, then redraws NW,NE,NW,W at 268. Losing that generation
                // rewrote a terrain line and refused N again on 280 instead
                // of taking NE. Fresh routes and opening formation moves
                // retain their own refusal handler below.
                if (battleNetLandRegroupMove(unit)
                        && unit.battleNetChaseLegOpensCold()
                        && unit.battleNetAttackOp0OutOfRange()
                        && unit.battleNetPathStepsTaken() > 0) {
    ...
    /** Whether the unit can enter any adjacent compass square. */
    boolean battleNetHasFreeNeighbour(Unit unit) {
        if (unit == null) {
            return false;
        }
        int stride = world.battleNetMovementStride(unit);
    ...
    boolean usesBattleNetMovePace(Unit unit) {
        ...
        return unit.battleNetDoubleStep()
                // A player's Move used to fall back to presentation timing.
                // XOrc 1's death knight 1491 spent 19 visits on the stride that
                // script.bin finishes in 18, changing where later clicks land.
                || unit.battleNetPlayerCommandMove()
                || battleNetLandRegroupMove(unit)
