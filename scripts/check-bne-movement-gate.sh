...
missing = names - seen
if missing or tests != 97 or skipped or failures or errors:
    raise SystemExit(
        f"movement referee: expected 97/0/0/0 with every class present; "
        f"got {tests}/{skipped}/{failures}/{errors}, missing={sorted(missing)}"
    )
print("movement edge referee: 97 pass, 0 skipped")
PY
