...
# verified almost nothing. 1,410 of 3,067 tests skip that way. This prints the
...
if [[ "$wc2_ok" == 1 && "$pack_ok" == 1 && "$opus_ok" == 1 ]]; then
    ...
    printf '  Expect 3067 tests, normally 31 skipped on the classic CI source.\n'
    printf '  A full run takes about five minutes.\n'
    printf '  All three private playtest-save referees reduce that exact count to 28.\n'
    ...
    printf '  saves, the opt-in production smoke and six release-sensitive tests.\n'
elif [[ "$wc2_ok" == 1 ]]; then
    ...
    printf '  with no input at all, 1410 of 3067 tests skip.\n'
