...
Success means: Real units complete formation detours and terrain-domain passages while 114 large-footprint, congestion, command-handoff and refusal checks pass with zero skips.
...
- The movement gate runs 114 focused checks with zero skips. Its naval referee records every commanded ship anchor's visual tile and raw terrain flags; transport coast permission is explicit while destroyers and tankers remain water-only.
