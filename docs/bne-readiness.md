...
Success means: Real units complete formation detours and terrain-domain passages while the focused movement and terrain rules pass.
...
- The movement gate runs focused checks with zero skips. Its naval referee records every commanded ship anchor's visual tile and raw terrain flags; transport coast permission is explicit while destroyers and tankers remain water-only.
...
Success means: Every projectile visibly leaves its source, preserves its retail launch facing, advances on the retail beat, draws the native arc frames, applies its impact once and renders both flight and impact with zero skipped checks.
...
```text
scripts/run-tests.sh -pl desktop -am -Dtest=BattleNetProjectilePoolOrderTest,BattleNetMissileMotionTest,CombatFeedbackTest,MissileRenderingTest,ImpactRenderingTest,NativeMissileRealDataTest -Dsurefire.failIfNoSpecifiedTests=false
```
...
Success means: The pinned retail dispatch and spell call shapes authenticate the implementation; representative offensive and buff spells spend the declared cost, apply their complete effects and use BNE's hit reaction with no skipped checks.
...
- The pinned Exorcism handler reaches retail HitUnit and its 0x0040a9d0 offer boundary before damage; the Java direct-spell path now uses that same offer-and-wait policy instead of layering LegacyEngine flee/attack-back on top.
- The BNE player/referee lane proves fireball, death coil, delayed Blizzard, death and decay, demolish, targeted casting, self casting and mana exhaustion; its native catalog sweep proves all 22 spell definitions and their effect argument trees load with no script tree, for 21 passing checks and zero skips.
...
Success means: Every retail binding matches its authority, unit selection/death/work events choose the complete legal sample groups, and a completed build renders audible PCM with zero skipped checks.
...
- All 371 sound bindings and all 398 voice-event bindings for 107 voiced units were sealed only after full legacy differentials, while every decoded clip comes from the retail BNE asset pack.
...
```text
scripts/run-tests.sh -pl engine,desktop -am -Dtest=SoundWithoutScriptsRealDataTest,UnitVoicesWithoutScriptsRealDataTest,SoundRealDataTest,SoundChoiceTest,BuilderReportsWorkCompleteTest,BuildingVoiceTest -Dsurefire.failIfNoSpecifiedTests=false
```
