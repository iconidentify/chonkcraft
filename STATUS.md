...
- Combat replacements still have an active playability defect: some units
  ...
  Its 42 new real-data tests cover replacement attacks, movement, Stop,
  save/load, weapon cooldowns, aggressor identity, blocked pursuit, dying
  targets, splash range, regrouping, scout arrival, worker damage timing and
  movement-program advancement while a retarget is queued;
  all pass with the authenticated pack and are part of the expanded 84-test
  ...
  release gate. The completed candidate suite has 3,079 tests, the same
