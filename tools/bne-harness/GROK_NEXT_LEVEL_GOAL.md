...
4. Read the five current substantive command debts and all missing generated/native cells. Treat 129/134 comparable as the starting measurement, not 129/240 completion. The current five are Attack residual offset, two Patrol cadence/offset rows, Repair terminal delay, and Return Goods resource lifecycle; rerun the worklist rather than trusting this prose if HEAD moves.

Perpetual three-lane round:
A. PLAYER TRANSACTIONS — close the highest-volume upstream missing or divergent cell from physical gesture -> ordered selection -> target interpretation -> exact wire fan-out -> acknowledgement/refusal -> first progress -> terminal outcome. Extend native UI-handler capture and the 27 InSight replay bridge. Prioritize authenticated group selections (1/3/9 mixed units), Shift queues, minimap/shore/tree/water/building target shapes, stand-ground and production. Attack-move is now 3/3 exact; expand it only with genuinely new target/group/queue shapes. Advance replay execution beyond its first unresolved identity instead of claiming the indexed tail ran. A generated or Java-only row is not native parity.
